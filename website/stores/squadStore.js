...
export const useSquadStore = create((set, get) => ({
  ...
  togglePlayer: (player) => {
    ...
    if (exists) {
      ...
      set({
        picks: [...picks, { playerId: player.id, isCaptain: picks.length === 0, player }],
      });
    }
  },
  ...
  getBudgetUsed: () =>
    get().picks.reduce((sum, p) => sum + (p.player?.fantasyValue || 0), 0),

  getBudgetCap: () => BUDGET_CAP,
  getCaptainCount: () => get().picks.filter((p) => p.isCaptain).length,

  isValid: () => {
    const picks = get().picks;
    if (picks.length < 5 || picks.length > 11) return false;
    if (get().getCaptainCount() !== 1) return false;
